...
from cfg import CLIPFeaturePredictor
...
def main():
    ...
    print(f"Total Parameters: {param_size / 1e6:.2f} M")
    ...
    predictor = CLIPFeaturePredictor()
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                img_p_m = transform(img_p.copy())
                image_tensor = get_tensor()(img_p)
                image_tensor = image_tensor.unsqueeze(0)
                ref_p = Image.open(opt.reference_path).convert("RGB").resize((224,224))
                ref_p = transform(ref_p)
                logits, _ = mask_model(ref_p.unsqueeze(0), opt.target_region)
                mask_p = torch.sigmoid(logits)
                mask_p = mask_p > 0.5
                mask_tensor_p = torch.from_numpy(mask_p)

                logits, _ = mask_model(img_p_m.unsqueeze(0), opt.target_region)
                mask_o = torch.sigmoid(logits)
                mask_o = mask_o > 0.5
                mask_o_opp = ~mask_o
                mask_tensor_o = torch.from_numpy(mask_o)
                mask_tensor_o_oppo = torch.from_numpy(mask_o_opp)
                inpaint_image = image_tensor*mask_tensor_o_oppo
                test_model_kwargs={}
                ...
                ref_tensor_r = predictor.replace(ref_p*mask_tensor_p, img_p*mask_tensor_o, opt.attribute)  # CFG conditional vector
                ...
                c = model.get_learned_conditioning(ref_tensor_r.to(torch.float16))
                c = model.proj_out(c)
